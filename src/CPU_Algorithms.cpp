...
std::pair<cv::Mat, double> OtsuCPU::execute(const cv::Mat& inputImage) {
    ...
    for (int i = 0; i < gray.rows; ++i) {
        for (int j = 0; j < gray.cols; ++j) histogram[gray.at<uchar>(i, j)]++;
    }
...
std::pair<cv::Mat, double> KMeansCPU::execute(const cv::Mat& inputImage) {
    ...
    for (int i = 0; i < data.rows; ++i) result.at<cv::Vec3f>(i, 0) = centroids[labels[i]];
...
std::pair<cv::Mat, double> FCMCPU::execute(const cv::Mat& inputImage) {
    ...
    while (iter < maxIterations) {
        ...
        for (int j = 0; j < k; ++j) {
            ...
            for (int i = 0; i < numPoints; ++i) { float u_m = std::pow(U[i][j], m); num += data.at<cv::Vec3f>(i, 0) * u_m; den += u_m; }
            if (den > 0) newCentroids[j] = num / den;
        }
        float maxDiff = 0.0f;
        for (int i = 0; i < numPoints; ++i) {
            for (int j = 0; j < k; ++j) {
                ...
                float sumD = 0.0f;
                for (int c = 0; c < k; ++c) { float dC = cv::norm(data.at<cv::Vec3f>(i, 0) - newCentroids[c]); if (dC > 0) sumD += std::pow(dist / dC, 2.0f / (m - 1.0f)); }
    ...
    for (int i = 0; i < numPoints; ++i) {
        ...
        for (int j = 0; j < k; ++j) if (U[i][j] > maxU) { maxU = U[i][j]; best = j; }
    ...
    for (int iter = 0; iter < maxIterations; ++iter) {
        ...
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < cols; ++j) {
                ...
                if (weight > 0) next.at<cv::Vec3f>(i, j) = sumC / weight;
            }
        }
        result = next;
    }
